...
- Reconcile the plugin's endless save log line with `specs/endless-mode.md` §3.8: the line carries a `skipped=` field that the specification's log-line list does not have. Decide whether the field joins the specification or leaves the log line.
