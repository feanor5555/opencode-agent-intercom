...
- A message that arrives between two tool steps rather than inside one is still unproven by an end-to-end run and needs its own driver, since the existing message driver only covers the inside-a-tool-call case.
