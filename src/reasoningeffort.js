...
//
// One fixed ladder (`low`/`medium`/`high`) serves every family: no per-model
...
// through into `output.options` already.

// The three ladder steps that mean an override. `default` is the absence of a
// value and never reaches this module.
const LADDER = new Set(["low", "medium", "high"])

// api.npm -> the patch for that family, built fresh per call so a caller
// mutating the returned object cannot alter the table.
const FAMILIES = {
  ...
  "@ai-sdk/google": (e) => ({ thinkingConfig: { thinkingLevel: e, includeThoughts: true } }),
  "@ai-sdk/google-vertex": (e) => ({ thinkingConfig: { thinkingLevel: e, includeThoughts: true } }),
...
// that does not declare `capabilities.reasoning === true`, a missing model,
// and a provider package the table does not name. Writing a key blindly for an
// unknown family would put an unrecognised field into that provider's request
// body, so silence is the answer there.
